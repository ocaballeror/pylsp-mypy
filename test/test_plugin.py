...
def test_dmypy_retries_on_connection_refused(last_diagnostics_monkeypatch, workspace):
    last_diagnostics_monkeypatch.setattr(
        FakeConfig,
        "plugin_settings",
        lambda _, p: ({"dmypy": True, "live_mode": False} if p == "pylsp_mypy" else {}),
    ...
    responses = [
        Mock(returncode=0, stdout="", stderr=""),  # status
        Mock(returncode=2, stdout="", stderr="[Errno 61] Connection refused\n"),  # first run
        Mock(returncode=0, stdout="", stderr=""),  # retry run
    ]
    m = Mock(side_effect=responses)
    last_diagnostics_monkeypatch.setattr(plugin.subprocess, "run", m)
    sleep = Mock()
    last_diagnostics_monkeypatch.setattr(plugin.time, "sleep", sleep)
    ...
    assert "run" in m.call_args_list[1].args[0]
    assert "run" in m.call_args_list[2].args[0]
    assert "restart" not in m.call_args_list[2].args[0]
